...
if [[ "$1" == "master" ]]; then 
	...
	ssh deploy@rey.webhouse.net 'sudo /srv/nodejs/senti/services/cloudfunctions/production/scripts/service-restart.sh master'
	echo
...
if [[ "$1" == "dev" ]]; then 
	...
	echo
	echo Restarting Senti Cloud Functions service: $1 ... 
	ssh deploy@rey.webhouse.net 'sudo /srv/nodejs/senti/services/cloudfunctions/development/scripts/service-restart.sh dev'
	echo
	echo Deployment to Senti Cloud Functions $1 and restart done!
	exit 0
fi